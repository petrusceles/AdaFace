...
def fuse_features_with_norm(
    stacked_embeddings, stacked_norms, fusion_method="norm_weighted_avg"
):
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(
        "./tinyface_result", args.model_name, "fusion_{}".format(args.fusion_method)
    )
    ...
    print(results)
    pd.DataFrame({"rank": [1, 5, 20], "values": results}).to_csv(
        os.path.join(save_path, "result.csv")
    )
